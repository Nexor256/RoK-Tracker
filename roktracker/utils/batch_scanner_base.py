...
class BatchScannerBase:
    """
    Shared implementation for the Alliance/Honor/Seed batch scanners.

    Subclasses configure the scanner-specific bits via class attributes:
      - scanner_name: used for the scan output folder ("scans_<lower>") and
        the output filename prefix (e.g. "Alliance").
      - ui_settings: the UI positions class (e.g. AllianceUI).
      - govs_per_screen: how many governor rows are visible per screen.
      - has_last_screen_variant: whether ui_settings exposes distinct
        name_normal/name_last + score_normal/score_last regions and an
        OCR-based "is this the last screen" check. When False, ui_settings
        is expected to expose plain `name`/`score` region lists instead.
      - save_trim_and_sum: whether the final save() call should trim the
        result to `amount` rows and append a totals row (used by Seed).
    """

    scanner_name: str = ""
    ui_settings: Type = None  # type: ignore[assignment]
    govs_per_screen: int = 6
    has_last_screen_variant: bool = True
    save_trim_and_sum: bool = False
    ...
    def __init__(self, port: int, config: FullConfig):
        ...
        self.scan_path = Path(self.root_dir / f"scans_{self.scanner_name.lower()}")
    ...
    def process_screen(self, image: MatLike, position: int) -> GovImageGroup:
        ui = self.ui_settings

        if self.has_last_screen_variant:
            if not self.reached_bottom:
                name_region = ui.name_normal[position]
                score_region = ui.score_normal[position]
            else:
                name_region = ui.name_last[position]
                score_region = ui.score_last[position]
        else:
            name_region = ui.name[position]
            score_region = ui.score[position]

        # fmt: off
        gov_name_im = cropToRegion(image, name_region)
        gov_name_im_bw = preprocessImage(
            gov_name_im, 3, ui.misc.threshold,
            12, ui.misc.invert,
        )

        gov_name_im_bw_small = preprocessImage(
            gov_name_im, 1, ui.misc.threshold,
            4, ui.misc.invert,
        )

        gov_score_im = cropToRegion(image, score_region)
        gov_score_im_bw = preprocessImage(
            gov_score_im, 3, ui.misc.threshold,
            12, ui.misc.invert,
        )
        # fmt: on
    ...
    def start_scan(self, kingdom: str, amount: int, formats: FormatsConfig):
        ...
        for i in range(0, self.screens_needed):
            ...
            if self.reached_bottom:
                ...
                self.adb_client.adb_send_events("Touch", self.ui_settings.misc.script)
                wait_random_range(1, self.max_random_delay)

        if self.save_trim_and_sum:
